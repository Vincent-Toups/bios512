...
def refresh_nonce(session: requests.Session, referer_url: str, delay: float = 0.8) -> str | None:
    ...
    # Only sleep if this GET wasn't served from cache
...
def main():
    ...
    # Read cURL
    if args.curl_file == "-":
        # If running interactively, prompt the user so they know to paste input
        if sys.stdin.isatty():
            print(
                "Paste the seed cURL command, then press Ctrl-D (Ctrl-Z+Enter on Windows) to continue...",
                file=sys.stderr,
            )
        raw_curl = sys.stdin.read()
        if not raw_curl.strip():
            sys.exit(
                "No cURL provided. Supply via --curl-file <path> or paste it on stdin."
            )
    else:
        ...
        if (r.status_code == 400 or r.status_code == 403 or r.status_code == 200) and try_nonce_refresh:
            ...
            if bad_nonce:
                ...
                if new_nonce:
                    ...
                    # Try again once (refresh_nonce already slept iff uncached)
                    r = robust_post(session, url, headers=headers, data=data)
        # Only sleep if this POST wasn't served from cache
        if not getattr(r, "from_cache", False):
    ...
    with open(args.out, fmode, newline="", encoding="utf-8", buffering=1) as csvfile:
